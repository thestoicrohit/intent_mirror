/**
 * Intent Mirror — Wealth Hub (B2C)
 * Your launchpad beyond the FD: live NSE/BSE stocks, mutual funds,
 * insurance, financial literacy, and a personal investment pathway
 * matched to your money persona.
 */
...
/* ─── Mutual Funds ───────────────────────────────────────────────────── */
const MUTUAL_FUNDS = [
  { name:'Mirae Asset Large Cap Fund', category:'Large Cap', returns1y:'+18.4%', returns3y:'+14.2%', risk:'Low-Medium', minSIP:'₹500',  rating:5, color:'#6ABFA0', tag:'Most Popular' },
  { name:'Parag Parikh Flexi Cap',     category:'Flexi Cap', returns1y:'+22.1%', returns3y:'+16.8%', risk:'Medium',     minSIP:'₹1,000',rating:5, color:'#5B9ED6', tag:'Top Rated'   },
  { name:'Axis Long Term Equity (ELSS)',category:'ELSS',     returns1y:'+19.7%', returns3y:'+15.1%', risk:'Medium',     minSIP:'₹500',  rating:4, color:'#9B8FD6', tag:'Tax Saver'   },
  { name:'SBI Bluechip Fund',          category:'Large Cap', returns1y:'+16.8%', returns3y:'+12.9%', risk:'Low',        minSIP:'₹500',  rating:4, color:'#D4A535', tag:'Safe Choice' },
  { name:'HDFC Mid-Cap Opportunities', category:'Mid Cap',   returns1y:'+31.2%', returns3y:'+19.4%', risk:'High',       minSIP:'₹1,000',rating:5, color:'#E05A3A', tag:'High Returns'},
  { name:'Nippon India Liquid Fund',   category:'Liquid',    returns1y:'+7.2%',  returns3y:'+6.8%',  risk:'Very Low',   minSIP:'₹100',  rating:4, color:'#56C4C4', tag:'FD Alternative'},
]

/* ─── Insurance / Protection Products ───────────────────────────────── */
const INSURANCE = [
  { name:'LIC Jeevan Anand',     type:'Life Insurance', provider:'LIC',        premium:'₹8,000/yr',  cover:'₹25L',  tenure:'21 yrs', color:'#6ABFA0', icon:'🛡️', tag:'Best Seller', desc:'Guaranteed returns + life cover. Great if you want safety with a savings habit.' },
  { name:'HDFC Click 2 Protect', type:'Term Insurance', provider:'HDFC Life',  premium:'₹12,000/yr', cover:'₹1 Cr', tenure:'30 yrs', color:'#5B9ED6', icon:'💙', tag:'Best Value', desc:'Maximum life cover at lowest cost. Pure term, 99.4% claim settlement.' },
  { name:'Star Health Comprehensive', type:'Health Insurance', provider:'Star Health', premium:'₹14,500/yr', cover:'₹10L', tenure:'1 yr', color:'#E05A3A', icon:'❤️', tag:'Top Rated', desc:'5,900+ cashless hospitals. No-claim bonus up to 50%. Family floater available.' },
  { name:'Bajaj Allianz Gold SIP',   type:'Gold Bond',      provider:'Bajaj Allianz', premium:'₹500/mo SIP', cover:'Market price', tenure:'Flexible', color:'#D4A535', icon:'🥇', tag:'Trending', desc:'Buy 24K digital gold from ₹500. No storage cost. Sell anytime.' },
]

/* ─── Financial Literacy tips ────────────────────────────────────────── */
const LITERACY_CARDS = [
  { icon:'💡', title:'FD vs Debt Mutual Fund', color:'#6ABFA0',
    content:"Debt MFs offer similar safety to FDs but better post-tax returns. On ₹5L for 3 years: FD gives ~₹6.1L, Debt MF ~₹6.4L after tax." },
  { icon:'📊', title:'The Power of SIP', color:'#5B9ED6',
    content:"₹5,000/month for 20 years in a large-cap fund (12% CAGR) = ₹49.9 Lakhs. The same in an FD (7%) = ₹26.3 Lakhs. A ₹23.6 Lakh difference from identical savings." },
  { icon:'🛡️', title:'The 50-30-20 Rule', color:'#D4A535',
    content:"50% of income → needs, 30% → wants, 20% → investments. Most salaried Indians invest only 8–10%. Doubling it is how real wealth gets built." },
  { icon:'🥇', title:'Why Gold Still Makes Sense', color:'#D4A535',
    content:"Gold has returned ~13% CAGR over 10 years in INR. A 10–15% allocation (digital gold or SGBs) is a sensible hedge against uncertainty." },
  { icon:'📋', title:'ELSS: Tax + Returns', color:'#9B8FD6',
    content:"ELSS funds save up to ₹46,800 in tax (30% bracket) under 80C, with the lowest lock-in (3 years). Average returns: 15–18% over 5 years." },
  { icon:'🏥', title:'Mind the Health Insurance Gap', color:'#E05A3A',
    content:"Average hospitalisation costs ₹2–3L, yet 70% of Indians have no health cover. One hospital stay can wipe out years of savings. Aim for ₹10L+ cover." },
]

/* ─── Persona → Investment pathway ───────────────────────────────────── */
...
function StockRow({ s, idx, c, live }) {
  ...
  return (
    <div style={{
      display: 'grid', gridTemplateColumns: '28px 70px 1fr 90px 80px 80px',
    ...
    >
      ...
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: c.text, fontVariantNumeric: 'tabular-nums' }}>₹{price.toFixed(2)}</div>
      </div>
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: col }}>{up ? '+' : ''}{change.toFixed(2)}</div>
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 1 }}>
          {[...Array(5)].map((_, i) => <span key={i} style={{ fontSize: 9, color: i < f.rating ? '#D4A535' : c.border }}>★</span>)}
        </div>
      </div>
      ...
      <button style={{
        width: '100%', marginTop: 10, padding: '7px', background: `${f.color}18`,
        border: `1px solid ${f.color}40`, borderRadius: 7, color: f.color,
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 10, background: `${p.color}18`, color: p.color, fontWeight: 700 }}>{p.tag}</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
        {[['Premium', p.premium], ['Cover', p.cover], ['Tenure', p.tenure]].map(([l, v]) => (
          <div key={l} style={{ flex: 1, background: `${p.color}10`, borderRadius: 7, padding: '7px 8px', textAlign: 'center' }}>
      ...
      <button style={{
        width: '100%', padding: '7px', background: `${p.color}18`,
        border: `1px solid ${p.color}40`, borderRadius: 7, color: p.color,
        fontSize: 11, fontWeight: 700, cursor: 'pointer',
      }}>Explore this cover →</button>
    </div>
  )
}
...
export default function WealthHub() {
  ...
  const myPersona = deriveMoneyPersona().persona
  ...
  return (
    <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {[
            ['12%+', tw.stat1, '#6ABFA0'],
            ['₹500', tw.stat2, '#5B9ED6'],
            ['3 yrs', tw.stat3, '#D4A535'],
          ].map(([val, lbl, col]) => (
            <div key={lbl} style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 10, padding: '10px 16px', textAlign: 'center' }}>
          ...
          {BASE_STOCKS.map(s => {
            ...
            return (
              <div key={s.symbol} style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: col, fontVariantNumeric: 'tabular-nums' }}>₹{price.toFixed(2)}</span>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {['stocks', 'mf', 'insurance', 'literacy', 'pathway'].map(id => (
          <button key={id} style={tabBtn(id)} onClick={() => setActiveTab(id)}>{tw.tabs[id]}</button>
        ))}
      </div>
      ...
      {activeTab === 'stocks' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}>
          <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '28px 70px 1fr 90px 80px 80px', padding: '7px 14px', background: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.03)', borderBottom: `1px solid ${c.border}` }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 12, padding: '16px' }}>
              ...
              {[
                ...
                <div key={idx.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: `1px solid ${c.border}` }}>
      ...
      {activeTab === 'insurance' && (
        <div>
          <div style={{ marginBottom: 14, padding: '12px 16px', background: 'rgba(224,89,58,0.06)', border: '1px solid rgba(224,89,58,0.2)', borderRadius: 10, fontSize: 12, color: c.textMuted }}>
            <strong style={{ color: c.text }}>🛡️ Protect before you grow:</strong> Only <strong style={{ color: '#E05A3A' }}>30% of Indians</strong> have health insurance. One hospital stay can cost more than a year of FD interest — cover the downside first, then invest with peace of mind.
          </div>
      ...
      {activeTab === 'literacy' && (
        <div>
          <div style={{ marginBottom: 14, padding: '12px 16px', background: `rgba(91,158,214,0.07)`, border: `1px solid rgba(91,158,214,0.2)`, borderRadius: 10, fontSize: 12, color: c.textMuted }}>
            <strong style={{ color: c.text }}>📚 2-minute reads that pay off:</strong> Understanding <em>why</em> a product fits you is what turns saving into wealth. Here are the ideas that matter most — no jargon.
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 14 }}>
            {LITERACY_CARDS.map((card, i) => (
              ...
              >
                ...
                <button style={{ marginTop: 12, width: '100%', padding: '7px', background: `${card.color}12`, border: `1px solid ${card.color}30`, borderRadius: 7, color: card.color, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>Learn more →</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeTab === 'pathway' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ padding: '12px 16px', background: `rgba(91,158,214,0.07)`, border: `1px solid rgba(91,158,214,0.2)`, borderRadius: 10, fontSize: 12, color: c.textMuted }}>
            {tw.pathwayIntro(myPersona)}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12 }}>
            {Object.entries(PERSONA_INVEST).map(([persona, info]) => {
              const mine = persona === myPersona
              return (
                <div key={persona} style={{
                  background: c.card, border: `1px solid ${mine ? info.color : c.border}`,
                  borderRadius: 12, padding: '16px', borderTop: `3px solid ${info.color}`,
                  boxShadow: mine ? `0 0 0 1px ${info.color}55` : 'none',
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <div style={{ fontSize: 10, color: info.color, fontWeight: 800, letterSpacing: 0.5 }}>{persona.toUpperCase()}</div>
                    {mine && <span style={{ fontSize: 8, fontWeight: 800, padding: '2px 6px', borderRadius: 4, background: `${info.color}22`, color: info.color }}>YOU</span>}
                  </div>
